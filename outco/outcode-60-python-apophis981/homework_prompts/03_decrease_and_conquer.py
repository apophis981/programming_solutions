...
def number_of_ones(arr):
    ...
    first = find_first(0, len(arr)-1, arr)
...
def closest_value(arr, target):
    ...
    def helper(left, right):
        ...
        if abs(target - arr[middle]) < abs(target - closest):
            closest = arr[middle]
...
def test():
    example = multiplication_russian_peasant(734, 487)
